...
async function sendPrompt(input: string) {
  const response = await openai.createImageEdit(
    // Have to add "as unknown as File" because OpenAI's type def is expecting a File object
    fs.createReadStream('./resources/imgs/test.png') as unknown as File,
    input,
    fs.createReadStream('./resources/imgs/test-mask.png') as unknown as File,
    1,
    '1024x1024'
  );
